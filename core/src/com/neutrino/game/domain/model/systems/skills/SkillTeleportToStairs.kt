...
class SkillTeleportToStairs(override val character: Character): Skill.ActiveSkill {
    ...
    override fun use() {
        ...
        for (y in 0 until LevelArrays.getLevel().map.yMax) {
            for (x in 0 until LevelArrays.getLevel().map.xMax) {
                for (z in 0 until LevelArrays.getLevel().map.map[y][x].size) {
                    if (LevelArrays.getLevel().map.map[y][x][z] is DungeonStairsDown)
                        stairsDown = Coord.get(x, y)
                    if (LevelArrays.getLevel().map.map[y][x][z] is DungeonStairsUp)
