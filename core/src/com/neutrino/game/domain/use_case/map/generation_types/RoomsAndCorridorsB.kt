...
class RoomsAndCorridorsB(level: Level, interpretedTags: TagInterpretation): MapGenerator(level, interpretedTags) {

    override fun generate(): List<List<MutableList<Entity>>> {
        val squidGeneration = SquidGeneration(level)

        squidGeneration.generateDungeon(TilesetType.ROOMS_AND_CORRIDORS_B)
